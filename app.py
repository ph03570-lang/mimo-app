...
# 인터넷 창 주소에서 구글 시트 주소를 자동으로 알아채는 기능
...
try:
    ...
    # 제 오타(SPREADSSHEET_URL)를 완벽하게 지우고 수정한 줄입니다!
    input_url = st.text_input("import streamlit as st
from google.oauth2.service_account import Credentials
import gspread
import pandas as pd
from datetime import datetime
...
try:
    ...
    if input_url and input_url != SPREADSHEET_URL:
...
with st.form("memo_form", clear_on_submit=True):
    ...
    memo_content = st.text_area("메모 내용", placeholder="여기에 메모 را 작성하세요...")
        ...
        with st.container(border=True):
            ...
            if st.session_state.get(f"editing_{idx}", False):
                with st.form(key=f"edit_form_{idx}"):
                    ...
                    col1, col:", value=SPREADSHEET_URL)
    if input_url and input_url != SPREADSHEET_URL:
        st.query_params["sheet_url"] = input_url
        st.rerun()
    st.stop()
